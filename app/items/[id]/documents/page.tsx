...
export default async function ItemDocumentsPage({
  ...
  const documents = await getDocumentsByRelation("item", itemId)
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Documentation for {item.name}</p>
          </div>
        </div>
        <Link href={`/documentation/new?item=${itemId}`}>
          <Button>
      ...
      {documents.length > 0 ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-muted-foreground mb-4">There are no documents associated with this item yet</p>
            <Link href={`/documentation/new?item=${itemId}`}>
              <Button>
                <Plus className="mr-2 h-4 w-4" />
                New Document
