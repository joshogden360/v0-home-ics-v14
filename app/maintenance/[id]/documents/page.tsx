import { getMaintenanceById } from "@/lib/actions/maintenance"
...
export default async function MaintenanceDocumentsPage({
  params,
}: {
  params: { id: string }
}) {
  ...
  const documents = await getDocumentsByRelation("maintenance", maintenanceId)

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link href={`/maintenance/${maintenanceId}`}>
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Documentation for {maintenance.maintenance_type} - {maintenance.item_name}
            </p>
          </div>
        </div>
        <Link href={`/documentation/new?maintenance=${maintenanceId}`}>
          <Button>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              There are no documents associated with this maintenance task yet
            </p>
            <Link href={`/documentation/new?maintenance=${maintenanceId}`}>
              <Button>
                <Plus className="mr-2 h-4 w-4" />
